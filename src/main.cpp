...
int main(int argc, char *argv[]) {
  ...
  if (!DatabaseManager::instance().init(dbPath)) {
    ToolTipOverlay::instance()->showText(QCursor::pos(),
            IconHelper::getIconHtml("close", "#e74c3c") + " <b style='color: #e74c3c;'>启动失败</b><br>无法初始化数据库！请检查写入权限或 SQLite 驱动。", 5000, QColor("#e74c3c"));
      ...
      QObject::connect(tool, &ScreenshotTool::screenshotCaptured, [=](const QImage& img, bool isOcrRequest){
        ...
        if (isOcrRequest) {
          ...
          if (!currentContent.isEmpty() && currentContent != initialContent) {
            ...
            ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 已从库中恢复识别结果并复制");
            }
            return;
          }
        ...
        QTimer::singleShot(500, [=](){
          ...
          if (text.trimmed().isEmpty()) {
            qWarning() << "[Acquire] 剪贴板为空，采集失败。";
                        ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("close", "#e74c3c") + " 未能采集到内容，请确保已选中浏览器中的文本");
            return;
          }
          ...
          // 成功反馈 (ToolTip)
                    QString feedback = IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 已采集灵感: " + (title.length() > 20 ? title.left(17) + "..." : title);
...
#endif
                ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 已纯净粘贴文本");
      }
    }
  });
