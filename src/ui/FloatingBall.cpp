...
void FloatingBall::contextMenuEvent(QContextMenuEvent* event) {
    ...
    QMenu* skinMenu = menu.addMenu(IconHelper::getIcon("palette"), "切换外观");
    skinMenu->setStyleSheet(menu.styleSheet());
    // 2026-03-xx 按照用户要求，颜色已强制绑定，不再使用硬编码皮肤色
    skinMenu->addAction(IconHelper::getIcon("coffee"), "摩卡·勃艮第", [this](){ switchSkin("mocha"); });
    skinMenu->addAction(IconHelper::getIcon("grid"), "经典黑金", [this](){ switchSkin("classic"); });
    skinMenu->addAction(IconHelper::getIcon("book"), "皇家蓝", [this](){ switchSkin("royal"); });
    skinMenu->addAction(IconHelper::getIcon("leaf"), "抹茶绿", [this](){ switchSkin("matcha"); });
    skinMenu->addAction(IconHelper::getIcon("book_open"), "摊开手稿", [this](){ switchSkin("open"); });
    skinMenu->addAction("默认天蓝", [this](){ switchSkin("default"); });

    menu.addSeparator();
    menu.addAction(IconHelper::getIcon("zap"), "打开快速笔记", this, &FloatingBall::requestQuickWindow);
    menu.addAction(IconHelper::getIcon("monitor"), "打开主界面", this, &FloatingBall::requestMainWindow);
    menu.addAction(IconHelper::getIcon("toolbox"), "打开工具箱", this, &FloatingBall::requestToolbox);
    menu.addAction(IconHelper::getIcon("add"), "新建灵感", this, &FloatingBall::requestNewIdea);
    menu.addSeparator();
    menu.addAction(IconHelper::getIcon("power"), "退出程序", [](){ qApp->quit(); });
...
void FloatingBall::dropEvent(QDropEvent* event) {
    ...
    // [CRITICAL] 拦截内部拖拽逻辑：严禁通过拖拽列表笔记到悬浮球来创建重复数据。
