...
void ScreenshotTool::autoSaveImage(const QImage& img) {
  ...
  if (img.save(fullPath)) {
    // 使用非阻塞彩色反馈告知用户已自动保存
    ToolTipOverlay::instance()->showText(QCursor::pos(), QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已自动保存至:\n%1").arg(fullPath));
  } else {
    ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("close", "#e74c3c") + "自动保存失败，请检查路径权限");
  }
}
