...
void ActivationDialog::onVerifyClicked() {
  ...
  if (DatabaseManager::instance().verifyActivationCode(key)) {
    ...
    updateRemainingAttempts();
    m_editKey->clear();
    ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 激活码错误</b>");
  }
}
...
void ActivationDialog::keyPressEvent(QKeyEvent* event) {
  ...
  if (event->key() == Qt::Key_F1) {
    ...
    ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 尝试次数已重置，请重新输入</b>");
    event->accept();
    return;
  }
