...
void MainWindow::initUI() {
    ...
    QString treeStyle = R"(
        ...
        /* 针对我的分类标题进行加粗白色处理 (1:1 同步 ArcMeta) */
    ...
    // 2026-03-xx 按照用户要求：将缩进由 16 降至 12 (与 ArcMeta 对齐)
    ...
    safeExpandPartitionTree(); // [USER_REQUEST] 1:1 同步 ArcMeta 安全展开策略
...
void MainWindow::safeExpandPartitionTree() {
    // 2026-03-xx 按照用户要求：物理级预防上锁分类展开 (1:1 同步 ArcMeta 逻辑)
    // 递归遍历，仅对未上锁的项执行展开。
...
void MainWindow::onSelectionChanged(const QItemSelection& selected, const QItemSelection& deselected) {
    ...
    // 则无论是在 MainWindow 还是 ArcMeta 切换选中，预览窗都必须立即同步内容。
    if (!indices.isEmpty()) {
        auto* preview = QuickPreview::instance();
...
void MainWindow::updateShortcuts() {
    ...
    // Let's fix the member variable usage to match NoteEditWindow/ArcMeta style
        ...
        if (key == Qt::Key_F2) {
            if (watched == m_partitionTree) {
                ...
                if (current.isValid() && current.data(CategoryModel::TypeRole).toString() == "category") {
                    // [CRITICAL] 锁定：统一使用行内编辑模式，严禁改为弹出对话框，以保持与 ArcMeta 的逻辑一致性
...
void MainWindow::saveLayout() {
    QSettings settings("ArcMeta", "MainWindow");
