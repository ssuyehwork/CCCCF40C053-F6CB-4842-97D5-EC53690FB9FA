  ...
  connect(m_noteList, &QListView::doubleClicked, this, [this](const QModelIndex& index){
    ...
    if (isExplicitPath || isAbsoluteTextPath) {
      ...
      if (QFileInfo::exists(fullPath)) {
        ...
        ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 文件已丢失：<br></b>" + fullPath);
      }
      return;
    }
...
void MainWindow::showContextMenu(const QPoint& pos) {
  ...
  if (selCount == 1) {
    ...
    menu.addAction(IconHelper::getIcon("calendar", "#4facfe", 18), "生成待办事项", [this, selected]() {
      ...
      ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已成功转化为待办事项");
    });
  }
