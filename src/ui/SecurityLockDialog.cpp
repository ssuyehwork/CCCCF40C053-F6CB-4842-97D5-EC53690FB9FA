...
void SecurityLockDialog::showRescueInput() {
  ...
  if (status["rescue_failed_attempts"].toInt() >= 4) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 今日抢救次数已耗尽</b>");
    return;
  }
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    if (DatabaseManager::instance().verifyRescueKey(key)) {
      ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 抢救成功，数据已完成自愈同步</b>");
      accept(); // 关闭锁定窗口，允许程序继续
    } else {
      updateAttemptsDisplay();
      ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 密钥验证失败</b>");
    }
  }
}
