  ...
  connect(dlg, &FramelessInputDialog::accepted, [this, oldName, dlg](){
    ...
    if (!newName.isEmpty() && newName != oldName) {
      if (DatabaseManager::instance().renameTagGlobally(oldName, newName)) {
        ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "标签已重命名并同步至所有笔记");
        refreshData();
      }
    }
  });
...
void TagManagerWindow::handleDelete() {
  ...
  connect(dlg, &FramelessMessageBox::confirmed, [this, tagNames](){
    ...
    if (successCount > 0) {
      ToolTipOverlay::instance()->showText(QCursor::pos(), tagNames.size() == 1 ? IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "标签已移除" : QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已批量移除 %1 个标签").arg(successCount));
      refreshData();
    } else {
      ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("close", "#e74c3c") + "移除失败，请检查数据库连接");
    }
  });
  dlg->show();
}
