...
void KeywordSearchWidget::copySelectedFiles() {
  ...
  ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已复制到剪贴板");
}
...
#endif
  QApplication::clipboard()->setMimeData(mimeData);
  ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已剪切到剪贴板");
}
...
void KeywordSearchWidget::onDeleteFile() {
  ...
  if (successCount > 0) ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已移至回收站");
}
...
void KeywordSearchWidget::onMergeFiles(const QStringList& filePaths, const QString& rootPath) {
  ...
  outFile.close();
  ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已保存: " + outName);
}
...
void KeywordSearchWidget::onSearch() {
  ...
  if (rootDir.isEmpty() || keyword.isEmpty()) {
    QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 目录和查找内容不能为空!</b>"), this);
    return;
  }
  ...
  (void)QtConcurrent::run([this, rootDir, keyword, replaceText, filter, caseSensitive]() {
    ...
    QMetaObject::invokeMethod(this, [this, modifiedFiles]() {
      ...
      QToolTip::showText(QCursor::pos(),
        StringUtils::wrapToolTip(QString("<b style='color: #2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 已修改 %1 个文件 (备份于 %2)</b>")
...
void KeywordSearchWidget::onUndo() {
  if (m_lastBackupPath.isEmpty() || !QDir(m_lastBackupPath).exists()) {
    QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 未找到有效的备份目录！</b>"), this);
    return;
  }
  ...
  QToolTip::showText(QCursor::pos(),
    StringUtils::wrapToolTip(QString("<b style='color: #2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 已恢复 %1 个文件</b>").arg(restored)), this);
}
