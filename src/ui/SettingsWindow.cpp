...
QWidget* SettingsWindow::createActivationPage() {
  ...
  if (isActive) {
    ...
    auto* lblActivated = new QLabel(QString("<div align='center'><b style='color: #2ecc71; font-size: 16px;'>IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") 已成功激活</b><br><br><span style='color: #a0a0a0; font-size: 16px; font-family: monospace; letter-spacing: 2px;'>%1</span></div>").arg(masked));
...
void SettingsWindow::onVerifySecretKey() {
  ...
  if (DatabaseManager::instance().verifyActivationCode(key)) {
    ...
    ToolTipOverlay::instance()->showText(QCursor::pos(),
      "<b style='color: #2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 激活成功，感谢支持！</b>", 5000, QColor("#2ecc71"));
    ...
    ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 密钥错误，激活失败</b>");
...
void SettingsWindow::onRemovePassword() {
  ...
  if (ok && input == realPwd) {
    ...
    ToolTipOverlay::instance()->showText(QCursor::pos(),
      "<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 密码错误，无法移除</b>");
  }
}
...
void SettingsWindow::onSaveClicked() {
  ...
  ToolTipOverlay::instance()->showText(QCursor::pos(),
    "<b style='color: #2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 设置已保存并立即生效</b>");
}
