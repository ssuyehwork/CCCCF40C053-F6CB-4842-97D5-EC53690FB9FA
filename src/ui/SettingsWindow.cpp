...
void SettingsWindow::saveSettings() {
    ...
    QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("✔ 设置已保存并立即生效"), this);
}
...
void SettingsWindow::handleSetPassword() {
    ...
    if (dlg->exec() == QDialog::Accepted) {
        if (dlg->password() == dlg->confirmPassword()) {
            ...
            QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("✔ 启动密码已设置"), this);
            ...
            QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("✖ 两次输入的密码不一致"), this);
        }
    }
    dlg->deleteLater();
}
        ...
        if (verifyDlg->text() == s.value("appPassword").toString()) {
            ...
            if (dlg->exec() == QDialog::Accepted) {
                if (dlg->password() == dlg->confirmPassword()) {
                    s.setValue("appPassword", dlg->password());
                    s.setValue("appPasswordHint", dlg->passwordHint());
                    QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("✔ 启动密码已更新"), this);
                } else {
                    QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("✖ 两次输入的密码不一致"), this);
                }
            }
            ...
            QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("✖ 密码错误，无法修改"), this);
        }
    }
...
void SettingsWindow::handleRemovePassword() {
    ...
    if (verifyDlg->exec() == QDialog::Accepted) {
        ...
        if (verifyDlg->text() == s.value("appPassword").toString()) {
            ...
            QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("✔ 启动密码已移除"), this);
            ...
            QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("✖ 密码错误，操作取消"), this);
        }
    }
    verifyDlg->deleteLater();
}
