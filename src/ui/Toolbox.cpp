...
void Toolbox::dropEvent(QDropEvent* event) {
  ...
  if (!localPaths.isEmpty()) {
    ...
    if (count > 0) {
      ToolTipOverlay::instance()->showText(QCursor::pos(), QString("<b style='color:#2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 已导入 %1 个项目到当前分类</b>").arg(count));
    }
    event->acceptProposedAction();
    return;
  }
  ...
  if (!content.isEmpty() || !dataBlob.isEmpty()) {
    ...
    ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color:#2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 已通过工具箱创建新灵感</b>");
    event->acceptProposedAction();
  }
}
