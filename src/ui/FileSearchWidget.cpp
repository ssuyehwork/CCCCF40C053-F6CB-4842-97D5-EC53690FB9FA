...
void FileSearchWidget::initUI() {
  ...
  connect(btnCopyAll, &QToolButton::clicked, this, [this](){
    ...
    ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color:#e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 结果列表为空</b>"), 2000);
      return;
    }
    QStringList paths;
  ...
  ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color:#2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 已复制全部搜索结果</b>"), 2000);
  });
...
void FileSearchWidget::onEditFile() {
  auto selectedItems = m_fileList->selectedItems();
  if (selectedItems.isEmpty()) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color:#e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 请先选择要操作的内容</b>"), 2000);
    return;
  }
...
void FileSearchWidget::copySelectedFiles() {
  ...
  QString msg = selectedItems.size() > 1 ? QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已复制 %1 个文件").arg(selectedItems.size()) : IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已复制到剪贴板";
...
void FileSearchWidget::onCutFile() {
  ...
  QString msg = selectedItems.size() > 1 ? QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已剪切 %1 个文件").arg(selectedItems.size()) : IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已剪切到剪贴板";
  ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip(QString("<b style='color: #2ecc71;'>%1</b>").arg(msg)));
}
...
void FileSearchWidget::onDeleteFile() {
  ...
  if (successCount > 0) {
    QString msg = selectedItems.size() > 1 ? QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "%1 个文件已移至回收站").arg(successCount) : IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "文件已移至回收站";
    ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip(QString("<b style='color: #2ecc71;'>%1</b>").arg(msg)));
    m_infoLabel->setText(msg);
  } else if (!selectedItems.isEmpty()) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 无法删除文件，请检查是否被占用</b>"));
  }
}
...
void FileSearchWidget::onMergeFiles(const QStringList& filePaths, const QString& rootPath) {
  if (filePaths.isEmpty()) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color:#e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 没有可合并的文件</b>"), 2000);
    return;
  }
  ...
  if (!outFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color:#e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 无法创建输出文件</b>"), 2000);
    return;
  }
  ...
  QString msg = QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已保存: %1 (%2个文件)").arg(outName).arg(filePaths.size());
...
void FileSearchWidget::onMergeSelectedFiles() {
  ...
  if (paths.isEmpty()) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color:#e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 选中项中没有支持的文件类型</b>"), 2000);
    return;
  }
...
void FileSearchWidget::onMergeFolderContent() {
  ...
  if (paths.isEmpty()) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color:#e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 目录中没有支持的文件类型</b>"), 2000);
    return;
  }
