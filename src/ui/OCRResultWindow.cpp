...
void OCRResultWindow::setRecognizedText(const QString& text, int contextId) {
  ...
  if (m_autoCopyCheck->isChecked()) {
    if (!isVisible()) {
      ...
      ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "识别完成并已复制到剪贴板");
    }
    onCopyClicked();
  } else if (!isVisible()) {
    ...
    ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "识别已完成并存入灵感库");
  }
}
