...
void QuickWindow::initUI() {
    ...
    connect(m_btnAutoCat, &QPushButton::clicked, this, [this](bool checked){
        ...
        if (checked) {
            ...
            ToolTipOverlay::instance()->showText(QCursor::pos(), QString("✅ 自动归档已开启 （%1）").arg(catName));
        } else {
            ToolTipOverlay::instance()->showText(QCursor::pos(), "❌ 自动归档已关闭");
        }
    });
...
void QuickWindow::setupShortcuts() {
    ...
    add("qw_show_all", [this](){
        ...
        ToolTipOverlay::instance()->showText(QCursor::pos(), "✔ 已切换至全部数据");
    });
