...
void QuickWindow::initUI() {
  ...
  connect(m_listView, &CleanListView::internalMoveRequested, this, [this](const QList<int>& ids, int row){
    if (m_currentFilterType == "recently_visited" || m_currentFilterType == "trash") {
      ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("help", "#f1c40f") + "当前视图不支持手动排序");
      return;
    }
  ...
  // 辅助函数：从 ShortcutManager 获取格式化后的快捷键字符串 (例如: " (Ctrl + Q)")
  ...
  // 辅助函数：创建图标按钮，支持旋转
  auto createToolBtn = [getScHint](QString iconName, QString color, QString tooltip, QString scId = "", int rotate = 0) {
  ...
  connect(m_btnAutoCat, &QPushButton::clicked, this, [this](bool checked){
    ...
    if (checked) {
      ...
      ToolTipOverlay::instance()->showText(QCursor::pos(), QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "自动归档已开启 （%1）").arg(catName));
    } else {
      ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("close", "#e74c3c") + "自动归档已关闭");
    }
  });
...
void QuickWindow::setupShortcuts() {
  ...
  add("qw_show_all", [this](){
    ...
    ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已切换至全部数据");
  });
...
void QuickWindow::activateNote(const QModelIndex& index) {
  ...
  if (itemType == "image") {
    ...
    if (fi.exists()) {
      ...
      ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("help", "#f1c40f") + "文件已丢失或被移动");
    }
    ...
    if (!validUrls.isEmpty()) {
      ...
      if (!missingFiles.isEmpty()) {
        ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("help", "#f1c40f") + "原文件已丢失，已复制路径文本");
      }
    }
  } else {
  ...
  if (physical || inTrash) {
    ...
    if (msg.exec() == QDialog::Accepted) {
      if (!idsToDelete.isEmpty()) {
        ...
        refreshData();
        refreshSidebar();
        ToolTipOverlay::instance()->showText(QCursor::pos(), QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已永久删除 %1 条数据").arg(idsToDelete.size()));
      }
    }
  } else {
    // 移至回收站：解除绑定
...
void QuickWindow::doGlobalLock() {
  ...
  if (settings.value("appPassword").toString().isEmpty()) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 尚未设定应用密码，请先进行设定</b>");
    return;
  }
  ...
  ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 应用已锁定</b>");
}
...
void QuickWindow::showSidebarMenu(const QPoint& pos) {
  ...
  if (type == "category") {
    ...
    menu.addAction(IconHelper::getIcon("branch", "#3498db", 18), "归类到此分类", [catId, currentName]() {
      ...
      ToolTipOverlay::instance()->showText(QCursor::pos(), QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已指定插件归类到: <b>%1</b>").arg(currentName));
    });
    menu.addSeparator();
    auto* importMenu = menu.addMenu(IconHelper::getIcon("file_import", "#1abc9c", 18), "导入数据");
    importMenu->setStyleSheet(menu.styleSheet());
    ...
    sortMenu->addAction("标题(当前层级) (A→Z)", [this, parentId]() {
      if (DatabaseManager::instance().reorderCategories(parentId, true))
        ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color:#2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 排列已完成</b>");
    });
    ...
    sortMenu->addAction("标题(全部) (A→Z)", [this]() {
      if (DatabaseManager::instance().reorderAllCategories(true))
        ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color:#2ecc71;'>" + IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + " 全部排列已完成</b>");
    });
      ...
      QTimer::singleShot(0, [this, catId]() {
        ...
        if (verifyDlg.exec() == QDialog::Accepted) {
          if (DatabaseManager::instance().verifyCategoryPassword(catId, verifyDlg.text())) {
            ...
            ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 旧密码验证失败</b>");
          }
        }
      });
    });
      ...
      QTimer::singleShot(0, [this, catId]() {
        ...
        if (dlg.exec() == QDialog::Accepted) {
          if (DatabaseManager::instance().verifyCategoryPassword(catId, dlg.text())) {
            ...
            ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #e74c3c;'>" + IconHelper::getIconHtml("close", "#e74c3c") + " 密码错误</b>");
          }
        }
      });
    });
...
void QuickWindow::handleTagInput() {
  ...
  m_tagEdit->clear();
  refreshData();
  ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "标签已添加");
}
...
void QuickWindow::openTagSelector() {
  ...
  connect(selector, &AdvancedTagSelector::tagsConfirmed, [this, selected](const QStringList& tags){
    ...
    refreshData();
    ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "标签已更新");
  });
...
void QuickWindow::doCopyTags() {
  ...
  ToolTipOverlay::instance()->showText(QCursor::pos(), QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已复制 %1 个标签").arg(tags.size()));
}
...
void QuickWindow::doOCR() {
  ...
  if (settings.value("autoCopy", false).toBool()) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("clock", "#f1c40f") + "正在识别文字...");
  } else {
    resWin->show();
  }
...
void QuickWindow::doPasteTags() {
  ...
  if (tagsToPaste.isEmpty()) {
    ToolTipOverlay::instance()->showText(QCursor::pos(), IconHelper::getIconHtml("close", "#e74c3c") + "标签剪贴板为空");
    return;
  }
  ...
  refreshData();
  ToolTipOverlay::instance()->showText(QCursor::pos(), QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "已覆盖粘贴标签至 %1 条数据").arg(selected.size()));
}
...
void QuickWindow::doExportCategory(int catId, const QString& catName) {
  ...
  if (progress) {
    ...
    delete progress;
    if (canceled) {
      ToolTipOverlay::instance()->showText(QCursor::pos(), "<b style='color: #e67e22;'>" + IconHelper::getIconHtml("help", "#f1c40f") + " 导出已取消</b>");
      return;
    }
  }
  ToolTipOverlay::instance()->showText(QCursor::pos(), QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "分类 [%1] 导出完成").arg(catName));
}
...
void QuickWindow::doImportCategory(int catId) {
  ...
  refreshData();
  refreshSidebar();
  ToolTipOverlay::instance()->showText(QCursor::pos(), QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "导入完成，共处理 %1 个项目").arg(totalCount));
}
...
void QuickWindow::doImportFolder(int catId) {
  ...
  refreshData();
  refreshSidebar();
  ToolTipOverlay::instance()->showText(QCursor::pos(), QString(IconHelper::getIconHtml("screenshot_confirm", "#2ecc71") + "文件夹导入完成，共处理 %1 个项目").arg(totalCount));
}
