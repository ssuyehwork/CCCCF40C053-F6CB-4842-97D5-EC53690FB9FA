...
DatabaseManager::DatabaseManager(QObject* parent) : QObject(parent) {
    ...
    m_autoSaveTimer->setInterval(7000); // 7秒自动同步间隔
...
bool DatabaseManager::init(const QString& dbPath) {
    ...
    m_dbPath = appDataPath + "/ArcMeta_kernel.db";
    ...
    QString connectionName = "ArcMeta_Main_Conn";
...
void DatabaseManager::handleAutoSave() {
    ...
    // 只有当距离最后一次操作超过 30 秒时，才执行保存，确保不打断用户的连续操作（如取色、采集）
    qint64 idleSecs = m_lastActivityTime.secsTo(QDateTime::currentDateTime());
    if (idleSecs < 30) {
        return;
    }

    qDebug() << "[DB] 监测到系统闲置已达" << idleSecs << "秒，开始执行背景保存...";
    m_isDirty = false;
...
void DatabaseManager::resetActivation() {
    ...
    // 3. 清理注册表锚点
    QSettings registry("HKEY_CURRENT_USER\\Software\\ArcMeta", QSettings::NativeFormat);
