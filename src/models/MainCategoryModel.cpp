...
MainCategoryModel::MainCategoryModel(Type type, QObject* parent) 
    : QStandardItemModel(parent), m_type(type) 
{
    ...
    connect(&DatabaseManager::instance(), &DatabaseManager::extensionTargetCategoryIdChanged, this, &MainCategoryModel::updateExtensionIcons);
    refresh();
}

void MainCategoryModel::refresh() {
    clear();
...
QVariant MainCategoryModel::data(const QModelIndex& index, int role) const {
...
bool MainCategoryModel::setData(const QModelIndex& index, const QVariant& value, int role) {
...
Qt::DropActions MainCategoryModel::supportedDropActions() const {
    return Qt::MoveAction;
}

bool MainCategoryModel::dropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent) {
    ...
    if (ok && action == Qt::MoveAction) {
        ...
        QTimer::singleShot(0, this, [persistentParent, this]() {
...
void MainCategoryModel::updateExtensionIcons() {
...
void MainCategoryModel::syncOrders(const QModelIndex& parent) {
