...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent, Qt::FramelessWindowHint) {
    ...
    connect(&DatabaseManager::instance(), &DatabaseManager::categoriesChanged, this, &MainWindow::refreshData, Qt::QueuedConnection);
...
void MainWindow::doDeleteSelected(bool physical) {
    ...
    if (physical || inTrash) {
        ...
        QToolTip::showText(QCursor::pos(), 
            StringUtils::wrapToolTip(QString("<b style='color: #2ecc71;'>✔ 已永久删除 %1 条数据</b>").arg(ids.count())), this);
    } else {
        QList<int> ids;
        for (const auto& index : std::as_const(selected)) ids << index.data(NoteModel::IdRole).toInt();
